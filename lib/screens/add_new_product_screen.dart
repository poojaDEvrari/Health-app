...
class _AddNewProductScreenState extends State<AddNewProductScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Add New Equipment', style: GoogleFonts.poppins(color: AppColors.primary, fontWeight: FontWeight.w700)),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  TextFormField(controller: _title, decoration: _input('e.g. Electric ICU Hospital Bed'), validator: (v)=> (v==null||v.isEmpty)?'Required':null),
                  const SizedBox(height: 16),
                  Text('Description', style: GoogleFonts.poppins(fontWeight: FontWeight.w700, color: Colors.black87)),
                  const SizedBox(height: 6),
                  TextFormField(controller: _description, decoration: _input('Enter a detailed description'), minLines: 3, maxLines: 5, validator: (v)=> (v==null||v.isEmpty)?'Required':null),
                  const SizedBox(height: 16),
                  Row(children: [
                    Expanded(child: TextFormField(controller: _price, keyboardType: TextInputType.number, decoration: _input('Price (₹)'), validator: (v)=> (v==null||v.isEmpty)?'Required':null)),
                    const SizedBox(width: 12),
                    Expanded(child: TextFormField(controller: _stock, keyboardType: TextInputType.number, decoration: _input('Stock'), validator: (v)=> (v==null||v.isEmpty)?'Required':null)),
                  ]),
                  const SizedBox(height: 16),
                  Row(children: [
